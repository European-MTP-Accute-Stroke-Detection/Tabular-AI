...
def preprocess_data(X_data,
    ...
    elif len(ordinal_features) > 0: # ANDREJ ENCODING
        #glmm5CV_encode_multiple_features_gpboost(Z_train, Z_val, Z_test, X_train, X_val, X_test, y_train, qs, RS):
        
        #qs=cardinalities_per_feature
        #Z=matrix woth features label encoded for categoricalfeatures
        Z_train = X_train.copy(deep=True)
        Z_valid = X_valid.copy(deep=True)
        Z_test = X_test.copy(deep=True)
        
        qs = []
        
        encoder = LabelEncoder()
        for column in ordinal_features:
            encoder = encoder.fit(pd.concat([Z_train[column], Z_valid[column], Z_test[column]]).values)
            Z_train[column] = encoder.transform(Z_train[column].values)
            Z_valid[column] = encoder.transform(Z_valid[column].values)
            Z_test[column] = encoder.transform(Z_test[column].values)  
            
            qs.append(len(encoder.classes_))
        
        X_train = X_train.drop(ordinal_features, axis=1)
        X_valid = X_valid.drop(ordinal_features, axis=1)
        X_test = X_test.drop(ordinal_features, axis=1)
        
        Z_train, Z_valid, Z_test = glmm5CV_encode_multiple_features_gpboost(Z_train[ordinal_features].values, Z_valid[ordinal_features].values, Z_test[ordinal_features].values, X_train, X_valid, X_test, y_train, qs, RS=42)
        
        X_train = pd.concat([X_train, pd.DataFrame(Z_train, index=X_train.index, columns=ordinal_features)], axis=1)
        X_valid = pd.concat([X_valid, pd.DataFrame(Z_valid, index=X_valid.index, columns=ordinal_features)], axis=1)
        X_test = pd.concat([X_test, pd.DataFrame(Z_test, index=X_test.index, columns=ordinal_features)], axis=1)
